class Api::PinsController < ApplicationController
  ...
  def index
    if(params[:page])
      @pins = Pin.all.includes(:author, :comments, :photo, :board).order("created_at DESC").page(params[:page]).per(15)
    elsif(params[:board_id])
      @pins = Pin.includes(:author, :comments, :photo, :board).where(board_id: params[:board_id]).order("created_at DESC")
    else
      @pins = Pin.all.includes(:author, :comments, :photo, :board).order("created_at DESC")
    end
  end
